...
async function send (sendInfo) {
  ...
  const info = await transporter.sendMail({
    ...
    subject: sendInfo.user !== '' ? `你好开发者，${sendInfo.user}! 账号注册码` : '账号注册码', // Subject line
    text: `您正在注册账号，邀请码是${sendInfo.code}，邀请码的过期时间:${sendInfo.expire}`, // plain text body
    html: `
    <div
      style="
    ...
    >
      ...
      <div style="padding: 25px">
        <div>
          您好，${sendInfo.user}同学，重置链接有效时间30分钟，请在${sendInfo.expire}之前重置您的密码:
        </div>
        <a
          href="${url}"
          style="
